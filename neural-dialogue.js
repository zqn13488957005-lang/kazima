/* LinguaReader dialogue TTS: Kokoro neural voice with system speech fallback. */
(()=>{
  ...
  window.nextDialogue=async function(){
    ...
    document.getElementById('dialogueProgress').textContent=`${dialogueIndex+1}/${dialogueLines.length} · ${gender==='male'?'👨 男声':'👩 女声'} · ${fallbackLang[l]||l} · Kokoro`;
    const finish=()=>{dialogueIndex++;dialogueTimer=setTimeout(()=>window.nextDialogue(),180)};
    if(window.KokoroTTS&&typeof window.KokoroTTS.speak==='function'){
      const ok=await window.KokoroTTS.speak(convertHanja(x.text,l),l,gender,finish);
      if(ok){document.getElementById('status').textContent=`正在使用 Kokoro 神经网络声音 ${dialogueIndex+1}/${dialogueLines.length}…`;return;}
    }
